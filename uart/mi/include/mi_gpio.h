...
//  Defines
//-------------------------------------------------------------------------------------------------
typedef void (*MI_GPIO_IsrCallback)(MI_OS_InterruptNum_e eIntr);
typedef void (*MI_GPIO_TaskCallback)(void);
//-------------------------------------------------------------------------------------------------
//  Structures
//-------------------------------------------------------------------------------------------------
typedef enum
{
    E_MI_GPIO_LEVEL_OFF = 0,        //LEVEL off iis equal to level low if not invert, but equal to level high if invert
    E_MI_GPIO_LEVEL_ON,             //LEVEL on is equal to level high if not invert, but equal to level low if invert
} MI_GPIO_Level_e;

typedef enum
{
    E_MI_GPIO_DIRECTION_OUTPUT = 0,
    E_MI_GPIO_DIRECTION_INPUT,
} MI_GPIO_Direction_e;

typedef struct MI_GPIO_EnableIntr_s
{
    MI_U8 u8GpioNum;
    MI_BOOL bPolarity;
    MI_GPIO_IsrCallback pfIsrCallbk;  //This callback is work within Isr, do emergency work only
    MI_GPIO_TaskCallback pfTaskCallbk;//This callback is work in Task, do anything after receiving intr here
} MI_GPIO_EnableIntr_t;

typedef struct MI_GPIO_InitParam_s
{
    MI_U32 u32Reserved;
} MI_GPIO_InitParam_t;
...
//------------------------------------------------------------------------------
MI_RESULT MI_GPIO_SetInvert(MI_U8 u8GpioNum, MI_BOOL bInvert);
...
/// @return MI_ERR_INVALID_PARAMETER: Parameters is invalid.
//------------------------------------------------------------------------------
MI_RESULT MI_GPIO_GetOnOff(MI_U8 u8GpioNum, MI_GPIO_Level_e *peLevel, MI_BOOL *pbInvert);
...
/// @return MI_ERR_INVALID_PARAMETER: Parameters is invalid.
//------------------------------------------------------------------------------
MI_RESULT MI_GPIO_GetDirection(MI_U8 u8GpioNum, MI_GPIO_Direction_e *peDirection);
